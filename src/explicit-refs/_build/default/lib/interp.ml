...
let rec eval_expr : expr -> exp_val ea_result = fun e ->
  ...
  | IsEqual (e1,e2) -> (*check that evaluation of e1, e2 are NumVals*)
      eval_expr e1 >>=
      int_of_numVal >>= fun ev1 ->
      eval_expr e2 >>= 
      int_of_numVal >>= fun ev2 ->
 ...
 | Proj(e, id) ->
    ...
    | RecordVal(fields) ->
        (try 
            let field_val = List.assoc id fields |> snd in
            match field_val with
            | RefVal loc -> Store.deref g_store loc (* Dereference if it's a RefVal *)
            | _ -> return field_val (* Directly return if it's not a RefVal *)
         with Not_found -> error ("Field " ^ id ^ " not found"))
  ...
  | SetField(e1, id, e2) ->
    ...
    | RecordVal(fields) ->
        eval_expr e2 >>= fun ev2 ->
        (match update_field fields id ev2 with
         | Ok updated_fields -> return (RecordVal(updated_fields))
         | Error _ -> error "Field not mutable")
    | _ -> error "Expected a record")
