...
let rec update_field fields id new_value = 
  match fields with
  | [] -> Ok [(id, (true, new_value))]  (* Assuming all fields are mutable *)
  | (field_id, (is_mutable, value)) :: tail ->
      if field_id = id then
        if is_mutable then
          Ok ((id, (true, new_value)) :: tail)
        else
          Error "Attempt to modify an immutable field"
      else
        match update_field tail id new_value with
        | Ok updated_tail -> Ok ((field_id, (is_mutable, value)) :: updated_tail)
